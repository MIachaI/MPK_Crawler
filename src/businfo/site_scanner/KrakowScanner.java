...
public class KrakowScanner extends SiteScanner {
    ...
    public ArrayList<BusStop> scan() throws IOException {
        ...
        for(BusStopLink busStopLink : busStopLinks){
            ...
            Document documentSite = DocHacker.getDocument(subSite);
            // get links and numbers elements
            Elements links = documentSite.select("table[style=' margin-bottom: 20px; '] tbody tr a[href]");
            Elements numbers = documentSite.select("table[style=' margin-bottom: 20px; '] tbody tr td[style=' text-align: right ;']");
